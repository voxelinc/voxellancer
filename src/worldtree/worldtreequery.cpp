...
bool WorldTreeQuery::areGeodesNear() {
    ...
    startQuery([&](WorldTreeGeode* geode) {
        result = true;
...
WorldTreeNode* WorldTreeQuery::containingNode() {
...
WorldTreeNode* WorldTreeQuery::getQueryRoot(WorldTreeNode* node) const {
    if (node == nullptr) {
        ...
        if (m_shape->containedBy(node->aabb())) {
            ...
            return getQueryRoot(node->parent());
        }
    }
}
...
void WorldTreeQuery::query(WorldTreeNode* node, std::function<void(WorldTreeGeode*)> onGeodeInteraction) {
    if (node->isLeaf()) {
        ...
        std::list<WorldTreeNode*> nearSubnodes;

        for (WorldTreeNode* subnode : node->subnodes()) {
            if (m_shape->nearTo(subnode->aabb())) {
                nearSubnodes.push_back(subnode);
            }
        }

        if (nearSubnodes.size() == 1) {
            m_containingNode = nearSubnodes.front();
        }

        for (WorldTreeNode* subnode : nearSubnodes) {
