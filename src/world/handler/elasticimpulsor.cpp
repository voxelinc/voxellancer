...
void ElasticImpulsor::parse(std::list<Impulse>& worldObjectImpulses) {
    for(Impulse& worldObjectImpulse : worldObjectImpulses) {
        ...
        Transform targetTransform(worldObject->transform(), physics.directionalSpeed(), glm::quat(physics.angularSpeed()));
        ...
        physics.setDirectionalSpeed(((m1 - m2) * v1 + 2.0f * v2 * m2) / (m1 + m2));
