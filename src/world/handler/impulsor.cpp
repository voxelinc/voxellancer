...
void Impulsor::parse(std::list<Impulse>& worldObjectImpulses) {
    for(Impulse& worldObjectImpulse : worldObjectImpulses) {
        ...
        glm::vec3 v1 = targetTransform.applyTo(glm::vec3(worldObjectImpulse.voxel()->cell())) - worldObjectImpulse.voxel()->position();
        ...
        glm::vec3 r = worldObject->transform().position() - worldObject->transform().applyTo(glm::vec3(worldObjectImpulse.voxel()->cell()));
