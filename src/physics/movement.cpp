...
void Movement::performStepped(const IAABB& phaseAABB) {
    ...
    std::unordered_set<WorldTreeGeode*> possibleColliders = WorldTreeQuery(m_collisionDetector.worldTree(), &phaseAABB, hint.node(), &m_worldObject.collisionFilter()).nearGeodes();
