...
glm::vec3 SimpleWayfind::calculateTravelPoint(WorldObject& object, glm::vec3 targetPoint) {
    ...
    std::unordered_set<WorldObject*> obstacles = WorldTreeQuery(&object.sector()->worldTree(), &capsule, object.collisionDetector().geode()->hint(), &filter).intersectingWorldObjects();
